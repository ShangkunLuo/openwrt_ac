...
function action_clock_status()
	...
	local tz = uci:get_first("system","system","timezone","CST-8")
        local lt = util.exec("export TZ="..tz.." && date \"+%Y-%m-%d %H:%M:%S %a\"")
        if not lt or "" == lt then lt = os.date() end

	luci.http.prepare_content("application/json")
	luci.http.write_json({ timestring = lt })
end
